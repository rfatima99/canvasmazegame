...
function ball(mx, my) {
  ...
  if (
    canvas.height / 2 - ballLoc.y > canvas.height / 2 - 10 &&
    x > canvas.width / 2 + 10
  ) {
    if (!won) {
      alert("you win");
  ...
  if (collision) {
    ...
    ballLoc = {
      x:
        Math.abs(0.1 * mx + 0.9 * x - x) < 2
          ? 0.1 * mx + 0.9 * x
          : x + 2 * Math.sign(0.1 * mx + 0.9 * x - x),
      y:
        Math.abs(0.1 * my + 0.9 * y - y) < 2
          ? 0.1 * my + 0.9 * y
          : y + 2 * Math.sign(0.1 * my + 0.9 * y - y),
    };
    ballMoves.push(ballLoc);
  }
